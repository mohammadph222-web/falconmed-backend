...
export async function initializeDatabase() {
  try {
    ...
    const result = await pool.query('SELECT COUNT(*) FROM patient_logs');
    ...
    if (patientCount === 0) {
      ...
      const waitingTime = (calledTime - arrivalTime) / 60000;  // ✅ موجب
      const serviceTime = (finishTime - calledTime) / 60000;   // ✅ موجب

      const patients = [
        'أحمد علي محمد', 'فاطمة محمد علي', 'محمد حسن خليفة', 'سارة خالد عمر',
        'علي محمود أحمد', 'نور الدين سالم', 'زينب أحمد الشامسية', 'خالد عبدالله محمد',
        'أم هانية حمد', 'سيف علي ناصر', 'منار حسين الشامسية', 'ياسر محمد علي',
        'ليلى خالد محمد', 'سامي عمر سليم', 'رشا أحمد علي', 'حمد ناصر محمود',
        'أسيل علي أحمد', 'فهد محمد سالم', 'جميلة حمد علي', 'خليل عبدالله محمد',
        'مريم علي ناصر', 'محمود حسن أحمد', 'سالمة محمد علي', 'نادر خالد محمود',
        'هناء أحمد عمر', 'جمعة علي سالم', 'لينة محمد خالد', 'إبراهيم حسن محمد',
        'رانية علي أحمد', 'عبدالرحمن محمود علي'
      ];

      for (let i = 1; i <= 30; i++) {
        const patientId = `P${String(i).padStart(3, '0')}`;
        const identified = i % 3 !== 0; // 2 من 3 معرّفين

        await pool.query(`
          INSERT INTO patient_logs 
          (patient_id, patient_name, branch_id, service_type, arrival_time, called_time, finish_time, identified, waiting_time_minutes, service_time_minutes)
      ...
      await pool.query(`
        ...
        VALUES 
          ('br_001', 'ph_001', CURRENT_DATE, 30, 21, 9, 18.70, 2.30, 90, 0.5),
      ...
      console.log(`✅ Database ready - ${patientCount} patients exist`);
    }
